...
// 2. 다른 풀이 방법
// 정렬을 통해, 접두사 관계에 있는 번호들이 인접하게 위치하도록 하고 인접한 번호들만 체크해서 접두어인 경우가 있는지 확인하는 방법. nlogn의 시간복잡도를 갖고 가장 빠른 시간 복잡도를 가진다.
// function solution(phone_book) {
//     // 1. 전화번호 배열을 정렬합니다.
//     phone_book.sort();
...
// 3. 세번째 푼 방식, phone_book 배열의 길이가 n일때, 시간 복잡도는 n^2. 여전히 시간 복잡도에서 시간초과가 뜨는 풀이
// function solution(phone_book) {
//     for (let i = 0; i < phone_book.length; i++) {
...
// 4. 두번째로 푼 방식으로, n이 phone_book 배열의 길이, m이 각 번호의 길이라고 했을때, 시간복잡도가(n^2 * m^2)로 매우 높게 나옴. 문제를 푸는 로직은 맞지만, 시간 효율성이 매우 떨어지는 방법
// function solution(phone_book) {
//     for (let i = 0; i < phone_book.length; i++) {
...
// 5. 맨 처음에 푼 방식으로, forEach의 작동 방식을 몰라서 리턴을 잘못함.
// function solution(phone_book) {
//     for (let i = 0; i < phone_book.length; i++) {
//         const mapOfNums = new Map();
